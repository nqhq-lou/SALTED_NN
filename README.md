...
This project is based on the open lightning template: [Lightning-Hydra-Template](https://github.com/ashleve/lightning-hydra-template).
Part of this README is inherited from the template.

## 📌  Introduction

Predict density fitting coefficients in the workflow of SALTED.

There are three kinds of models:
1. Input: $\lambda$-descriptor, MLP for each $\lambda$ channel and each species.
1. Input: $\lambda$-descriptor, E3 GNN for each species (pairs).
1. Input: auto embedding / MACE embedding, E3 GNN for each species (pairs).
...
│
├── .env                      <- For storing private environment variables
